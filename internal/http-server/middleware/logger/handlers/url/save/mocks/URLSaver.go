...
// SaveURL provides a mock function with given fields: alias, urlToSaVE
func (_m *URLSaver) SaveURL(alias string, urlToSaVE string) (int64, error) {
	ret := _m.Called(alias, urlToSaVE)
	...
	if rf, ok := ret.Get(0).(func(string, string) (int64, error)); ok {
		return rf(alias, urlToSaVE)
	}
	if rf, ok := ret.Get(0).(func(string, string) int64); ok {
		r0 = rf(alias, urlToSaVE)
	} else {
		r0 = ret.Get(0).(int64)
	}

	if rf, ok := ret.Get(1).(func(string, string) error); ok {
		r1 = rf(alias, urlToSaVE)
	} else {
		r1 = ret.Error(1)
	}
